...
const reportPath = new URL(
  '../../docs/skills/ai-safety-meme-creator/archive-stroke-wrap-comparison.html',
  import.meta.url
)
const reportRoute = '/archive-stroke-wrap-comparison.html'

let reportServer: Server | undefined
let reportUrl = process.env.MEME_STROKE_WRAP_COMPARISON_URL
...
test('V4/V5 report hides incomplete comparisons before image review', async ({
  page
}) => {
  ...
  await expect(list.locator('[data-version="v5"]')).toHaveCount(rowCount)
