...
`barlow-condensed-latin-800-normal.woff` is the Latin ExtraBold subset of Barlow Condensed v13, copied from `@fontsource/barlow-condensed` so Next.js can trace it into the scenario, source, franchise, risk-family, and concept social-image functions without depending on a runtime `node_modules` layout.
