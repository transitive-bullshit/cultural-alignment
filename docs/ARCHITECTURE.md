...
Schema v2 established the current slug baseline: it ignored v1 slug maps and regenerated every slug. Later syncs preserve the slug belonging to each surviving Notion page ID, allocate deterministic slugs for new IDs, and remove deleted IDs from the map so their former slugs may be reused. The checked manifest remains v2 until the first authenticated descriptor migration. That sync emits manifest schema v3, which retains the slug maps plus Notion source contracts, snapshot counts, and fixture IDs but no longer contains media entries; R2 descriptors then become the media-state authority.

The v2-to-v3 transition is seeded safely. When the checked-in manifest is still v2 and a descriptor is absent, the synchronizer scans the current image selection and may validate and reuse the matching legacy variant entry. It writes that record's descriptor before the staged v3 manifest can replace the snapshot. Consequently, a failed first migration run leaves the v2 manifest available for retry rather than committing a v3 manifest before its reusable state exists in R2.
