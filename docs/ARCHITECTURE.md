...
The full archive keeps texture residency bounded to visible and near-visible cards. The content count is intentionally independent of decoded GPU texture count. Placeholder and full-image stages share a hard 64-texture ceiling, prioritize the current center and incoming edge, and evict lower-priority records by recency; teardown disposes listeners, materials, geometry, and resident textures.
