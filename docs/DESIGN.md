...
Resource detail titles (concepts, risk families, sources, and franchises) share a restrained `clamp(44px, 6vw, 88px)` scale, reduced to `clamp(40px, 10vw, 64px)` at `860px` and below, where all resource detail headers stack into one column. Their `0.96` line height gives multiline titles breathing room; balanced wrapping uses whole words, with an emergency break only for a word wider than its layout column.
